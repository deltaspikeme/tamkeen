...
public class PersonalityTestGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all personalityTests").get("/api/personality-tests").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new personalityTest")
                        .post("/api/personality-tests")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"testName\": \"SAMPLE_TEXT\"" + ", \"description\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_personalityTest_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created personalityTest").get("${new_personalityTest_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created personalityTest").delete("${new_personalityTest_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PersonalityTest entity").exec(scn);
