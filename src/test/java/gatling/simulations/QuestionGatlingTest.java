...
public class QuestionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all questions").get("/api/questions").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new question")
                        .post("/api/questions")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"questionText\": null" + ", \"questionType\": \"I_E\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created question").get("${new_question_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created question").delete("${new_question_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
