...
public class TestResultGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all testResults").get("/api/test-results").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new testResult")
                        .post("/api/test-results")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"analysis\": null" + ", \"resultDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_testResult_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created testResult").get("${new_testResult_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created testResult").delete("${new_testResult_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TestResult entity").exec(scn);
