 ...
 */
public class AnswerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all answers").get("/api/answers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new answer")
                        .post("/api/answers")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"answerText\": \"SAMPLE_TEXT\"" + ", \"score\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created answer").get("${new_answer_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created answer").delete("${new_answer_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Answer entity").exec(scn);
