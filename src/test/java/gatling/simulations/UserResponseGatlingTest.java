...
public class UserResponseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userResponses").get("/api/user-responses").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userResponse")
                        .post("/api/user-responses")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"responseDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userResponse_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userResponse").get("${new_userResponse_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created userResponse").delete("${new_userResponse_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserResponse entity").exec(scn);
