 ...
 */
public class ConsultationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all consultations").get("/api/consultations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new consultation")
                        .post("/api/consultations")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"consultantName\": \"SAMPLE_TEXT\"" +
                                ", \"consultationDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"notes\": null" +
                                "}"
                            )
                        )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created consultation").get("${new_consultation_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created consultation").delete("${new_consultation_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Consultation entity").exec(scn);
